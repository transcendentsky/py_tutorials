...
class OctConv(nn.Module):
    def __init__(self, ch_in, ch_out, kernel_size, stride=1, alphas=(0.5, 0.5)):
        ...
        # FILTERS
        print(self.ch_out_hf, self.ch_in_hf, kernel_size, kernel_size)
        print(self.ch_out_hf, self.ch_in_lf, kernel_size, kernel_size)
        self.wHtoH = nn.Parameter(torch.ones(self.ch_out_hf, self.ch_in_hf, kernel_size, kernel_size))
        self.wHtoL = nn.Parameter(torch.randn(self.ch_out_lf, self.ch_in_hf, kernel_size, kernel_size))
        self.wLtoH = nn.Parameter(torch.ones(self.ch_out_hf, self.ch_in_lf, kernel_size, kernel_size))
